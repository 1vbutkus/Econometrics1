...
Pakete `datasets` yra duomenų masyvas `longley`. Iš šio kintamųjų padarykite tiesinę regresiją, 
kur prognozuojamas kintamasis būtų `GNP`, o regresosius `Employed`. Jums reikia:
...
Užduoties atlikimo terminas 2016-11-11 23:59.

## Task 12. (Atnaujinta)

Jūs turite patikrinti savo kolegų darbus. Faile `Studentai.xlsx` yra paskirstyta kuriuos darbus reikia tikrinti.
Tikrinimo vertinimus turite sukelti į internetinę formą adresu:

https://docs.google.com/forms/d/e/1FAIpQLSfU3pHAgrXh4oIBkSu_MmN037vCnhWC7p5HUfH6t3oX56utEA/viewform

Atkreipiu dėmesį, kad formoje užduočių sąrašas pailgėjo. Dabar jame turi būti visi darbai, kuriuos reikės tikrinti.

Kai ištikrinsite kolegų darbus, turite taip pat patikrinti ir užpidyti savo darbus. Jeigu darbe buvo klaidų - būkite sau kritiški ir pademonstruokite, kad kažką išmokote išvardindami savo klaidas.

Užduoties atlikimo terminas 2016-12-08 23:59.


## Pranešimas

Visos naujos užduotys bus formuluojamos virtualioje mokymosi aplinkoje (VMA). Prašau visus apsilankyti ir atlikti paskirtas užduotis.

## Lygties sprendimas sveikaisiais skaičiais

Reikia išspręsti lygti

$$xy = x + y + 5$$

sveikaisias skaičiais.

Sprendimas labai paprastas patikrinant visus variantus intervale $x \in [-20,20]$
...
ind = y%%1 == 0
ind[is.na(ind)] = FALSE
paste("x=",x[ind],"; y=", y[ind], sep="")
```

Gavus sprendinius dar reikia įsitikinti, kad daugiau sprendinių nėra. Isitikinti padeda toks grafikas:
```{r}
plot(x,y,type="l")
abline(h=2, col=2)
abline(h=0, col=2)
...
Tolimesni $y$ sprendiniai yra intervale $(0, 2)$. Vienintelis sveikasis skaičius šiame intervale yra $1$. 
Bet kai $y=1$, tai sprendinys neegzistuoja.
...
newx = seq(-3, 3, by=0.1)
df = data.frame(x=newx, d=1*(newx>0))
df['ypred'] = predict(mod, df)
lines(df$x, df$ypred, col=2)

summary(mod)

```

Pirmo pavyzdžio alternatyva
```{r}
mod = lm(y~x+I(x>0))
...
# teisiog lygtis
y = 3*x + rnorm(n, sd=0.5)

# sukuriam kategorini kintamaji
s = factor(sample(c('LT', 'LV', 'EE'), size = n, replace = TRUE), levels=c("LT", "LV", "EE"))
...
# standartinis vertinimas
lm(y~x + s)
# individualus (bet analogiskas) vertinimas
lm(y ~ x + I((s=='LT')*1) + I((s=='LV')*1) + I((s=='EE')*1) - 1)
